...
using (
    ...
    select 'LOGGER_VERSION'         PREF_NAME,  '1.2.0' PREF_VALUE from dual union
...
create or replace context logger_global_ctx using logger accessed globally;
...
as
    ...
    $ELSE
        ...
        $ELSE
            l_level := sys_context('logger_global_ctx','level');
    ...
    $ELSE
        ...
        $ELSE
            l_call_stack_pref := sys_context('logger_global_ctx','include_call_stack');
        ...
        $ELSE
            if ok_to_log(logger.g_debug) then
                if g_proc_start_times.exists(p_unit) then
                    ...
                    l_time_string := extract (second from (systimestamp - g_proc_start_times(p_unit)));
                    l_text := l_pad||'STOP : '||p_unit ||' - '||l_time_string||' seconds';
...
is
    ...
    l_version           constant number  := dbms_db_version.version + (dbms_db_version.version / 100);
begin
    ...
    /* ************************************************************************** */
    -- Do we have a synonym to and the privs to query apex.wwv_flow_data ?
    --
    begin
        execute immediate 'select 1 from wwv_flow_data where rownum = 1' into l_dummy;
